...
double ARMIJO_SIGMA=0.5;
double L0_ALPHA=1000;
double L0_BETA=0.75;
double ETA=0.5;
int NUM_PGD_ITERATIONS=10000;
...
inline double evalFunction(vector<double> & expected_counts ,vector<double> & current_point,vector<double> &alphas)
{
	...
	for (int i =0 ;i<num_elements;i++)
	{
		...
		{
			func_value += expected_counts[i] * log(current_point[i]) + alphas[i]* exp(-current_point[i]/L0_BETA);
		}
		else
		{
			func_value += alphas[i]* exp(-current_point[i]/L0_BETA);
		}
	}
...
void inline evalGradient(vector<double> & expected_counts, vector<double> & current_point, vector<double> & gradient,vector<double> & alphas)
{
	int num_elements = expected_counts.size();
	for (int i =0 ;i<num_elements;i++)
	{
		...
		if (current_point[i] != 0.0)
		{
			gradient[i] = -1 *(expected_counts[i] / current_point[i] - alphas[i]* exp(-current_point[i]/L0_BETA)/L0_BETA	);
		}
		else
		{
			gradient[i] = -1 *(- alphas[i]* exp(-current_point[i]/L0_BETA)/L0_BETA	);
		}
...
void projectedGradientDescentWithArmijoRule(vector<double> & expected_counts ,vector<double> & current_prob, vector<double> & new_prob, vector<double> & alphas,double beta,int num_pgd_iterations)
{
	NUM_PGD_ITERATIONS = num_pgd_iterations;
	L0_BETA = beta;
  cout << L0_ALPHA << " " << L0_BETA << " " <<NUM_PGD_ITERATIONS<<endl;
	...
	for (int time = 1; time <= NUM_PGD_ITERATIONS; time++)
	{
		...
		double current_function_value = evalFunction(expected_counts,current_point,alphas);
		vector<double> gradient (num_elements,(double) 0.0);
		evalGradient(expected_counts,current_point,gradient,alphas);
		//getchar();
...
/*         getchar(); */
		...
		//	current_beta = 
			...
			double func_value_at_temp_point = evalFunction(expected_counts,temp_point,alphas);
