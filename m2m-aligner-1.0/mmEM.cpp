...
long int global_index = 1; // lhuang
			...
			for (int yl = 0; yl < stringY[i].size(); yl++)
			{
				if (myParam.delX)
				{
					for (int j=0; (j < myParam.maxX) && (xl-j >= 0); j++)
					{
						...
						alphas[ssX][myParam.nullChar] = j;
						//cout<<"1 for "<<ssX<<" and "<<myParam.nullChar<<" the alpha is "<<alphas[ssX][myParam.nullChar]<<endl;
                        if (index[ssX][myParam.nullChar] == 0)
                          index[ssX][myParam.nullChar] = global_index++;
					}
				}
				...
				if (myParam.delY)
				{
					for (int k=0; (k < myParam.maxY) && (yl-k >=0); k++)
					{
						...
						alphas[myParam.nullChar][ssY] = k ;
						//cout<<"2 for "<<ssY<<" and "<<myParam.nullChar<<" the alpha is "<<alphas[myParam.nullChar][ssY]<<endl;
                        if (index[myParam.nullChar][ssY] == 0)
                          index[myParam.nullChar][ssY] = global_index++;
					}
				}
				...
				for (int j = 0; (j < myParam.maxX) && (xl-j >= 0); j++)
				{
					for (int k=0; (k < myParam.maxY) && (yl-k >=0); k++)
					{
						...
						//string ssY = stringY[i].substr(yl-k,k+1);
						string ssX = join(stringX[i], xl-j, j+1);
						string ssY = join(stringY[i], yl-k, k+1); // lhuang: bug: separator
						...
						//cout<<"3 for "<<ssY<<" and "<<ssX<<" the alpha is "<<alphas[ssX][ssY]<<endl;
						counts[ssX][ssY] = 1;
						alphas[ssX][ssY] = j + k ;
                        if (index[ssX][ssY] == 0)
                          index[ssX][ssY] = global_index++;
					}
				}
			}
		}
...
// 		{
//           cout << pos->first << " " << pos2->first << " "<< pos2->second <<endl;
//         }
//     }
//     getchar();
...
long double mmEM::maximization(param myParam)
{
	...
	for(hash_2StrDouble::iterator pos = counts.begin(); pos != counts.end(); pos++)
	{
		for (hash_StrDouble::iterator pos2 = (pos->second).begin(); pos2 != (pos->second).end(); pos2++)
		{
			...
			totalChange += fabs(probs[pos->first][pos2->first] - updateProb); // lhuang: change of prob
			newProbs[pos->first][pos2->first] = updateProb; // lhuang: assign new prob
		}
	}
...
//computing the probability of e | y
long double mmEM::computePairProb(param myParam, vector_str x, vector_str y)
{
  ...
  //returning the total prob of this pair
	return (alpha[x.size()][y.size()]);
}
			...
			if ( (xl > 0) && (yl > 0) )
			{
				for (int i = 1; (i <= myParam.maxX) && (xl-i >=0); i++)
				{
					for (int j = 1; (j <= myParam.maxY) && (yl-j >=0); j++)
					{
                      ...
                      // lhuang: never allow 2-2 or 3-3 or 4-4 mappings
                      if (! myParam.eqMap) // lhuang eqMap is by default False
						{
							if ((i==j) && (i>1))
							{
								continue;
                        ...
                        // lhuang: doubling probability trick
...
void mmEM::createAlignments(param myParam)
{
		...
		if (nScore.size() > 0)
		{
			...
			for (int nbest = 0; nbest < nScore.size(); nbest++)
			{
				...
				for (unsigned int k=0; k < nAlignY[nbest].size(); k++)
				{
                    ...
                    // lhuang
                    dict[nAlignX[0][k]][nAlignY[0][k]] = 1;
				}
    ...
    int dict_size = 0;
	for(hash_2StrDouble::iterator pos = dict.begin(); pos != dict.end(); pos++)
	{
		for (hash_StrDouble::iterator pos2 = (pos->second).begin(); pos2 != (pos->second).end(); pos2++)
		{
          if (dict[pos->first][pos2->first] > 0.5)
            dict_size += 1;
		}
	}
    cout << "Dictionary size "<< dict_size;
